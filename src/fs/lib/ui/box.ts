...
if (!sidebar) {
  ...
  sidebar.querySelector(".head")?.addEventListener("click", () => {
    ...
    setTimeout(
      () =>
        ...
        Object.assign(doc.querySelector(".monaco-editor")?.style, {
          width: "0px",
        }),
      255
    );
  ...
  win._boxEdgeDetect = () =>
    doc.querySelectorAll(".sb .box").forEach((box) =>
      Object.assign(box.style, {
        left:
          Math.max(
            Math.min(win.innerWidth - box.offsetWidth, box.offsetLeft),
            0
          ) + "px",
        top:
          Math.max(
            Math.min(win.innerHeight - box.offsetHeight, box.offsetTop),
            0
          ) + "px",
      })
    );
...
let createItem = (title, content, icon, ...classes) => {
  let item = sidebar.appendChild(
    elemFromHTML(
      `<div class="${classes.join(
        " "
      )}"><div class="head"><a class="icon">${icon}</a><span class=title>${title}</span><a class="icon collapser">\ueab7</a><a class="icon close">\ueab8</a></div><div class="body">${content}</div></div>`
    )
  );
  Object.assign(item, {
    ...
    log: (html, timestamp = true) => {
      if (!item.logTarget || !doc.contains(item.logTarget))
        item.logTarget = item.body.appendChild(
          elemFromHTML("<div class=log></div>")
        );
      let logEntry = item.logTarget.appendChild(
        elemFromHTML(`<p>${timestamp ? ts() : ""} ${html}</p>`)
      );
  ...
  [
    ...
    [
      "Float to Top",
      () =>
        sidebar.querySelector(".head").insertAdjacentElement("afterEnd", item),
  ...
  ].forEach((args) => item.addContextItem(...args));

  item.addEventListener(
    "mousedown",
    (e) =>
      item.classList.contains("box") &&
      Object.assign(item.style, { zIndex: zIndex() })
  );
  item.head.addEventListener("mousedown", (e) => {
    if (item.classList.contains("sbitem"))
      return e.button || transition(() => item.classList.toggle("c"));
    ...
    let boxDrag = (e) =>
      Object.assign(item.style, {
        left:
          Math.max(
            Math.min(win.innerWidth - item.offsetWidth, l + e.clientX - x),
            0
          ) + "px",
        top:
          Math.max(
            Math.min(win.innerHeight - item.offsetHeight, t + e.clientY - y),
            0
          ) + "px",
      });
    let boxDragEnd = (e) =>
      doc.removeEventListener("mouseup", boxDragEnd) ||
      doc.removeEventListener("mousemove", boxDrag);
    doc.addEventListener("mouseup", boxDragEnd) ||
      doc.addEventListener("mousemove", boxDrag);
  });
  item.head
    .querySelector(".close")
    .addEventListener("click", (e) => item["remove"]());
  item.head
    ...
    .addEventListener(
      ...
      (e) =>
        item.classList.contains("box") &&
        transition(() => item.classList.toggle("c") || win._boxEdgeDetect())
    );
  item.head.addEventListener(
    "contextmenu",
    (e) => e.preventDefault() || contextMenu(item, e.clientX, e.clientY)
  );
  Object.assign(item.style, {
    left: Math.floor(win.innerWidth / 2 - item.offsetWidth / 2) + "px",
    top: Math.floor(win.innerHeight / 2 - item.offsetHeight / 2) + "px",
...
export let createBox = (title, content, icon = "\uea74", ...classes) =>
  createItem(title, content, icon, ...classes, "box");
export let createReactBox = (title, content, icon = "\uea74", ...classes) =>
  createItem(title, content, icon, ...classes, "box", "box-react-root");
export let createSidebarItem = (title, content, icon = "\uea74", ...classes) =>
  createItem(title, content, icon, ...classes, "sbitem");
export let confirm = (text) => {
  ...
  box.addEventListener(
    ...
    (e) =>
      ...
      (e.key.toLowerCase() === "n" && box.querySelectorAll("button")[1].click())
  );
...
export let prompt = (text) => {
  ...
  box
    ...
    .addEventListener(
      "keyup",
      (e) => e.key == "Enter" && box.querySelector("button").click()
    );
  return new Promise((r) =>
    box
...
let contextMenu = (item, x, y) => {
  ...
  let options = Object.entries(item.contextItems).filter(([name, entry]) =>
    entry.cFn()
  );
  let box = createBox(
    "",
  ...
  Object.assign(box.style, {
    left:
      Math.max(
        Math.min(win.innerWidth - box.offsetWidth / 2, x),
        box.offsetWidth / 2
      ) + "px",
    top:
      Math.max(
        Math.min(win.innerHeight - box.offsetHeight / 2, y),
        box.offsetHeight / 2
      ) + "px",
  ...
  let docFunction = () =>
    box["remove"](doc.removeEventListener("mousedown", docFunction));
...
let zIndex = () =>
  Math.max(
    9000,
    ...[...doc.querySelectorAll(".sb .box")].map((box) => box.style.zIndex)
  ) + 1;
