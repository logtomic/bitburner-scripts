...
async function displayAsBox(ns: NS) {
  ...
  const textArea = box.querySelector("#fs-box-list") as HTMLElement;
  ...
  while (doc.body.contains(box)) {
    if (refresh) {
      ...
      lineElems.push(elemFromHTML("<span>|</span><span>|</span>") as Node);
      ...
      textArea?.replaceChildren(...(lineElems as Node[]));
    }
    await slp(1000);
  }
}

async function displayFiles(ns: NS, flags: Record<string, any>) {
